...
%% data_dir/0 from mcl_turn_credentials_service. mcl_om:boot/1 picks them up and starts
%% the store plus its evoq subscription BEFORE start/1 fires, so you never call
%% reckon_db_sup:start_store/1 yourself.
-module(mcl_turn_credentials_app).

-behaviour(application).

-export([start/2, stop/1]).

start(_Type, _Args) -> mcl_om:boot(mcl_turn_credentials_service).
